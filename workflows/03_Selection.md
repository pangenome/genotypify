...
bmws analyze $DIR_BASE/data/amy.vcf.gz $DIR_BASE/data/amy.meta -d diploid -l 4.5 -g 30 -n 100000 -t -o $DIR_BASE/amylase/bmws/amy.n100000.out
```
...
```R
​​s_hat <- read_tsv("/home/bioinfo26/Amylase_project/amy.s_hat.txt", col_names = "s") %>%
​​mutate(generation=388:1, time=(388-generation)*30)
s_hat_mean=mean(s_hat$s)

paths <- read_tsv("/home/bioinfo26/Amylase_project/amy.paths.tsv", col_names = FALSE) %>%                      
...
​​
### Dario's adjusts the generation to match the dataset size, ensuring correctness.
​s_ci <- read_tsv(str_c("/home/bioinfo26/Amylase_project/bmws/bootstrap/amy.s_hat.", 0:50, ".txt"), col_names = "s") %>%
​​mutate(generation = rep(388:1, each = 1000)[1:n()]) %>%
​​mutate(time = (388 - generation) * 30) %>%
​​group_by(time, generation) %>%
​​summarise(
​​  s_mean = mean(s),
​​  s_lower = quantile(s, 0.025),
​​  s_higher = quantile(s, 0.975))
​​  
​​  ##Nicolas one
